...
\newtcbox{\icoboxg}{on line, arc=3pt, colback=green!60!black, colframe=green!60!black, before upper={\rule[-3pt]{0pt}{10pt}}, boxrule=0.1pt, boxsep=0pt, left=3pt, right=3pt, top=1pt, bottom=.5pt}
\newcommand{\icog}[1]{\icoboxg{\color{white}\texttt{#1}}}
...
\begin{itemize}[itemsep=3em]
    ...
    Create a new account on {\color{blue} \url{https://www.overleaf.com}} using a \textbf{non-institutional email} by clicking on the \ico{Register} button located on the \textbf{top right}. Follow on-screen prompts. Once finished, you should be directed to an almost blank screen with a welcome message. 
    
    \textit{Note}: If your browser window is too small, the correct \ico{Register} button might be hidden behind the \ico{\faBars} icon. It is advised that you widen the window for the rest of the tutorial.
    ...
    \item[\inslabel{4}] In a separate browser tab, visit {\color{blue}\url{https://rb.gy/bejkvm}}.
    
    \newpage 
    \item[\inslabel{5}] 
    \begin{multicols}{2}
    Download the entire project by clicking on the green \ico{Download Code} button and then selecting the \ico{Download ZIP} option as shown in Figure \ref{fig:download}.
    ...
    \item[\inslabel{6}] 
    % \begin{multicols}{2}
    Uncompress the \verb|zip| file downloaded in step \textinslabel{5} into a separate folder. Your File Explorer (Windows) or Finder (macOS) should now look similar to figure \ref{fig:unzipped}.
    
    {
        \centering
        \includegraphics[width=0.9\columnwidth]{unzipped.png}
        \captionof{figure}{File explorer after unzipped the zipped project.}
        \label{fig:unzipped}
    }
    \vspace{-2em}
    % \end{multicols}
    ...
    \item[\inslabel{7}]
    % \begin{multicols}{2}
    Return to the Overleaf editor. Click on the \ico{\faUpload} upload icon located at the top left of the page, then upload all \textbf{eight} files unzipped from step \textinslabel{6} following the on screen instructions.
    % The file explorer located at the left of the screen should now look exactly the same as Figure \ref{fig:uploaded}.
    
    % {
    %     \centering
    %     \includegraphics[width=0.3\columnwidth]{files.png}
    ...
    {
        \centering
        \includegraphics[width=0.9\columnwidth]{compiler.png}
    ...
    \item[\inslabel{9}] Return to the project by clicking on the grayed out editor. Select the file \texttt{main.tex} in the Overleaf file explorer if it is not already highlighted. Push the green \icog{\faRefresh\ Recompile} button at the top of the screen, and wait for the \icog{\faRefresh\ Compiling...} animation to end. This document should now appear in your browser.
    
    \item[\inslabel{\small{10}}] Download the compiled \texttt{pdf} by clicking on the \ico{\faDownload} button located next to the Recompile button. Congratulations!
\end{itemize}
